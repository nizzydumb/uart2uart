    ...
    def __init__(self, toolchain="gowin", sys_clk_freq=48e6,
        ...
        **kwargs):
        ...
        SoCCore.__init__(self, platform, sys_clk_freq, ident="LiteX SoC on Tang Primer 20K", **kwargs)
       	...
       	# Here will be the logic of custom UART
       	
        uart_prompt_rx = Signal()
       	uart_prompt_tx = Signal()
       	
       	class _Pads: pass
        uart_prompt_pads = _Pads()
        uart_prompt_pads.rx = uart_prompt_rx
        uart_prompt_pads.tx = uart_prompt_tx
        
        self.submodules.uart_prompt_phy = UARTPHY(
            pads         = uart_prompt_pads,
            clk_freq = sys_clk_freq,
            baudrate     = 115200,
        )
        self.submodules.uart_prompt = UART(self.uart_prompt_phy)
        self.add_csr("uart_prompt")
       	
       	platform.add_source("hdl/uartprompt.v")
       	self.specials += Instance("uartprompt",
       	   i_clock = ClockSignal(),
       	    i_reset = ResetSignal(),
       	    o_tx = uart_prompt_rx,
       	    i_rx = uart_prompt_tx)  
        ...
        #    ("uart_prompt", 0,
        #    Subsignal("tx", Pins("D14")),  # FPGA pin for TX → внешний Rx
        #    Subsignal("rx", Pins("E15")),  # FPGA pin for RX → внешний Tx
        #    IOStandard("LVCMOS33")
        #    ),])
        
        #self.submodules.uart_prompt_phy = UARTPHY(
        #    pads = platform.request("uart_prompt"),
        #    clk_freq = sys_clk_freq,
        #    baudrate = 115200,
        #)
